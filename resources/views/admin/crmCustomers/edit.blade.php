...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.crm-customers.update", [$crmCustomer->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="client_code">{{ trans('cruds.crmCustomer.fields.client_code') }}</label>
                <input class="form-control {{ $errors->has('client_code') ? 'is-invalid' : '' }}" type="number" name="client_code" id="client_code" value="{{ old('client_code', $crmCustomer->client_code) }}" step="1">
                @if($errors->has('client_code'))
                    <div class="invalid-feedback">
                        {{ $errors->first('client_code') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.crmCustomer.fields.client_code_helper') }}</span>
            </div>
            <div class="form-group">
